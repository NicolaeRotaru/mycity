...
const FINTO_RCLONE = [
  ...
  // regola generica `--*`, altrimenti il numero finisce fra i percorsi e il
  // finto legge il file sbagliato.
  '    --count) shift 2;;',
  '    --*) shift;;',
  '    *) posizionali+=("$1"); shift;;',
  '  esac',
  'done',
  '# 8/9/2026 — Prima di dirsi riuscito, lo script va a RIPRENDERE una foto dalla',
  '# copia e la legge. Il finto deve saper rispondere anche a questo, e qui puo',
  '# farlo sul serio: la copia e una cartella vera su disco.',
  'if [ "$comando" = "lsf" ]; then',
