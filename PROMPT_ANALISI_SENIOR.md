...
| 6 | **SRE / DevOps** | Affidabilità, idempotenza, cron, deploy, rollback, scaling, observability | "Cosa fallisce alle 3 di notte? Il cron è idempotente se gira due volte? Il rate limit regge in multi-istanza? Come faccio rollback?" | `app/api/cron/**`, `vercel.json`, `instrumentation.ts`, `lib/rate-limit.ts`, Sentry config |
...
- **Domande:** Query **N+1** nelle pagine liste (catalogo, ordini, admin)? Le query calde hanno indici (vedi migrazioni `049/050/051/052`)? Il **rate limit in-memory** (`lib/rate-limit.ts`) regge su Vercel, dove ogni richiesta puo' finire su una copia diversa che parte coi contatori a zero (o serve Upstash)? Il **bundle client** è contenuto (`optimizePackageImports`, dynamic import della mappa)? Le **immagini** sono ottimizzate (Supabase render API, AVIF/WebP, `minimumCacheTTL` 1 anno)? Il **Realtime** (tracking rider, chat) ha un numero di subscription sostenibile? Caching/`revalidate` sulle pagine pubbliche?
...
- **Domande:** Sentry cattura errori server **e** client con contesto utile (e PII scrubbata)? PostHog/GA4 partono solo con consenso? C'è logging strutturato (`lib/logger.ts`)? Gli alert operativi (`cron/operational-alerts`) coprono i segnali giusti (ordini bloccati, payout falliti, code email)? `vercel.json` definisce bene regione, cron e tetti di durata delle funzioni? Le variabili d'ambiente sul progetto Vercel ci sono TUTTE (non «quasi tutte»)?
- **Dove:** `sentry.*.config.ts`, `instrumentation.ts`, `lib/analytics/**`, `lib/logger.ts`, `app/api/cron/operational-alerts`, `app/status`, `vercel.json`.
