...
export default function SellerDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border-2 border-olive-200 rounded-xl p-5">
        <h2 className="font-bold text-ink-900 mb-3 flex items-center gap-2">💰 Cassa</h2>
        <div className="grid grid-cols-3 gap-3 text-center">
          <div className="bg-olive-50 rounded-lg p-3">
            <p className="text-xs uppercase tracking-wide text-olive-700 font-semibold">Oggi</p>
            <p className="text-xl sm:text-2xl font-bold text-olive-900 mt-1">{formatPrice(stats.revenueToday)}</p>
            <p className="text-xs text-olive-600">{stats.ordersToday} articoli</p>
          </div>
          <div className="bg-primary-50 rounded-lg p-3">
            <p className="text-xs uppercase tracking-wide text-primary-800 font-semibold">7 giorni</p>
            <p className="text-xl sm:text-2xl font-bold text-primary-900 mt-1">{formatPrice(stats.revenue7)}</p>
            <p className="text-xs text-primary-700">{stats.orders7} articoli</p>
          </div>
          <div className="bg-secondary-50 rounded-lg p-3">
            <p className="text-xs uppercase tracking-wide text-secondary-700 font-semibold">30 giorni</p>
            <p className="text-xl sm:text-2xl font-bold text-secondary-900 mt-1">{formatPrice(stats.revenue30)}</p>
            <p className="text-xs text-secondary-600">{stats.last30Count} articoli</p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          {stats.availableCount < 5 && (
            <div className="bg-white rounded-lg p-4 border border-primary-100">
              <p className="font-bold text-primary-800 mb-1">📷 Aggiungi più prodotti</p>
              <p className="text-ink-600">I negozi con +10 prodotti vendono il 70% in più.</p>
              <Link href="/seller/products/new" className="text-primary-700 hover:underline text-xs font-semibold mt-2 inline-block">
                Aggiungi ora →
              </Link>
            </div>
          )}
          <div className="bg-white rounded-lg p-4 border border-primary-100">
            <p className="font-bold text-primary-800 mb-1">📸 Foto di qualità</p>
            <p className="text-ink-600">Prodotti con almeno 3 foto vendono 2× di più.</p>
          </div>
          <div className="bg-white rounded-lg p-4 border border-primary-100">
...
const accentClasses: Record<string, string> = {
  emerald: 'from-olive-500 to-teal-500',
  indigo:  'from-primary-500 to-purple-500',
  purple:  'from-purple-500 to-pink-500',
  amber:   'from-accent-400 to-orange-500',
};
...
const KpiCard = ({
  ...
  <div className="bg-white border rounded-xl p-5 relative overflow-hidden">
    ...
    <div className="text-2xl font-extrabold text-ink-800 relative">{value}</div>
    <div className="text-xs text-ink-500 mt-1 relative">{label}</div>
    {hint && <div className="text-[10px] text-ink-400 mt-2 relative">{hint}</div>}
  </div>
);
