...
export default function SellerReviewsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-900">⭐ Recensioni</h1>
        <p className="text-sm text-gray-500">Cosa pensano i clienti del tuo negozio</p>
      </div>

      {isLoading ? (
        <div className="text-center py-12 text-gray-400">Caricamento recensioni…</div>
      ) : reviews.length === 0 ? (
        <div className="bg-white border rounded-xl p-12 text-center">
          <div className="text-5xl mb-3">📝</div>
          <p className="font-semibold text-gray-700">Nessuna recensione ancora</p>
        ...
        <>
          ...
          <section className="bg-white border rounded-xl p-6 grid md:grid-cols-[200px_1fr] gap-6">
            <div className="text-center">
              <div className="text-6xl font-extrabold text-gray-900">{stats.avg.toFixed(1)}</div>
              <Stars rating={stats.avg} />
              <p className="text-sm text-gray-500 mt-1">{stats.count} recensioni</p>
            </div>
              ...
              {stats.distribution.map((count, i) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-2 py-1 rounded hover:bg-gray-50 transition-colors ${
                      filter === star ? 'bg-amber-50' : ''
                    }`}
                  >
          ...
          {filter !== 'all' && (
            <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 text-sm">
              ...
              <button onClick={() => setFilter('all')} className="text-amber-700 font-semibold hover:underline">
                Mostra tutte
              </button>
            </div>
          )}
  ...
  return (
    <article className="bg-white border rounded-xl p-5">
      <header className="flex items-start gap-3 mb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <p className="font-semibold text-gray-900">{review.reviewer?.full_name ?? 'Cliente'}</p>
      ...
      <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
        ...
        {review.order_id && (
          <span className="text-xs text-gray-400 font-mono">Ordine #{review.order_id.slice(0, 6).toUpperCase()}</span>
        )}
      </div>
        ...
        <div className="mt-3">
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button
              ...
              className="px-4 py-1.5 rounded text-sm bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold"
            >
