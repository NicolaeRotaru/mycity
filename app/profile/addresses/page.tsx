    ...
    <div className="container mx-auto px-4 py-8 max-w-2xl space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-1">I tuoi indirizzi</h1>
        </div>
        {!showForm && (
          <button
      ...
      {showForm && (
        ...
        >
          ...
          <div>
            <label className="text-sm font-medium text-gray-700">Etichetta</label>
            <input
              value={form.label}
          ...
          <div>
            <label className="text-sm font-medium text-gray-700">Note per il rider</label>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={save.isPending} className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-5 py-2 rounded font-semibold">
              {save.isPending ? 'Salvataggio…' : 'Salva'}
            </button>
            <button type="button" onClick={() => { setShowForm(false); setEditing(null); }} className="text-gray-600 hover:text-gray-900 px-3">
              Annulla
            </button>
          </div>
        </form>
            ...
            <div key={a.id} className="bg-white border rounded-xl p-4 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-bold text-gray-900 flex items-center gap-2">
                  📍 {a.label}
                  {a.is_default && <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-semibold">Predefinito</span>}
                </p>
                <p className="text-sm text-gray-700 mt-1">{a.full_name}</p>
                <p className="text-sm text-gray-600">{a.address}, {a.zip} {a.city}</p>
