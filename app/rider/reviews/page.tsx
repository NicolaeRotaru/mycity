...
export default function RiderReviewsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-extrabold text-gray-900">⭐ Le tue recensioni</h1>
        <p className="text-sm text-gray-500">Cosa pensano i clienti delle tue consegne</p>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <section className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-6 grid md:grid-cols-[200px_1fr] gap-6">
            <div className="text-center">
              <div className="text-6xl font-extrabold text-amber-900">{stats.avg.toFixed(1)}</div>
              <Stars rating={stats.avg} />
              <p className="text-sm text-amber-800 mt-1">{stats.count} recensioni</p>
              {stats.avg >= 4.5 && (
              ...
              {stats.distribution.map((count, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-xs text-amber-900 w-10">{count}</span>
                  </button>
                );
              })}
            </div>
            ...
            {filtered.map((r) => {
              ...
              return (
                <article key={r.id} className="bg-white border rounded-xl p-5">
                  <header className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        <p className="font-semibold text-gray-900">{r.reviewer?.full_name ?? 'Cliente'}</p>
                  ...
                  {r.comment && (
                    <p className="text-sm text-gray-700 leading-relaxed mt-2 pl-13">{r.comment}</p>
                  )}
                </article>
              );
            })}
